...
Dockertag (qcom-6.6.65-qli.1.4-ver.1.1_22.04), Dockertag taken based on the release folder in small letters(Docker will not allow Capital letters in Docker tag and appending with Dockerfile OS version for easy to identify the release build with dockerfile.
...
```{.sh}
  $ bash docker/docker_run.sh -t qcom-6.6.65-qli.1.4-ver.1.1_22.04 -r qcom-6.6.65-QLI.1.4-Ver.1.1
```
...
```{.sh}
  $ bash docker/docker_run.sh -t qcom-6.6.65-qli.1.4-ver.1.1_22.04 -r qcom-6.6.65-QLI.1.4-Ver.1.1 --build-override "base"
```

If you are facing issue with repo downloading from google, use below command for alternate repo

```{.sh}
  $ bash docker/docker_run.sh -t qcom-6.6.65-qli.1.4-ver.1.1_22.04 -r qcom-6.6.65-QLI.1.4-Ver.1.1 --alternate-repo true
```
